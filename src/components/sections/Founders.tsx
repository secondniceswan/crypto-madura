...
export default async function Founders() {
  ...
  return (
    // Full-bleed section-curtain wrapper, with section-container on an inner div —
    // section-container alone on the <section> (as before) confines its max-w-80rem
    // to the centered column, so the bg tint never reached the viewport edges above ~1280px.
    <section id="founder" className="section-curtain bg-bg-secondary">
        ...
        {founders.length === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 max-w-3xl mx-auto">
            {founders.map((founder, i) => (
              <Reveal key={founder.id ?? founder.name} delay={i * 100} variant="kinetic">
